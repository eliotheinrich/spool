...
Data is formatted in the blocks using a linked-list, where each piece of memory stored in a file is split between chunks, where a chunk is a contiguous 
region of memory with a 16-byte header. The first 8 bytes represent a pointer to the next chunk in the allocation, and the second 8 bytes represent the 
